 ...
 * 封装DBUtils的数据库操作类
 * 
 * @author <a href="http://www.ioxox.com">sain</a>
 *
 */
public class DbUtilsTemplate {
		...
		try {
			if (params == null) {
				list = (List<T>) queryRunner.query(sql, new BeanListHandler(
						entityClass, new BasicRowProcessor(new DBToBeanProcessor())));
			} else {
				list = (List<T>) queryRunner.query(sql, new BeanListHandler(
						entityClass, new BasicRowProcessor(new DBToBeanProcessor())), params);
			}
		} catch (SQLException e) {
			LOG.error("Error occured while attempting to query data", e);
		}
	...
	public <T> T findFirst(Class<T> entityClass, String sql, Object[] params) {
		...
		try {
			if (params == null) {
				object = queryRunner.query(sql, new BeanHandler(entityClass, new BasicRowProcessor(new DBToBeanProcessor())));
			} else {
				object = queryRunner.query(sql, new BeanHandler(entityClass, new BasicRowProcessor(new DBToBeanProcessor())),
						params);
			}
