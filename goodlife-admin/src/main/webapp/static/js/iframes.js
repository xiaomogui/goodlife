...
function ifreme_methei(mh) {
	mh=mh?mh:0;
	$('#mleft', parent.document).attr('style','');
	$('#mleft .floatl_box', parent.document).attr('style','');
	var m = $("body").height();
	var k = parseInt($("#mbox", parent.document).attr('jiluht'));
	var l = $('#mleft', parent.document).height()+35;
	l = m < l ? l : m;
	if (m < k && l < k) l = k;
	l=l+10;
	l=l<mh?mh:l;
	$('#mleft', parent.document).height(l);
	$('#mleft .floatl_box', parent.document).height(l);
	$('#mright', parent.document).height(l);
